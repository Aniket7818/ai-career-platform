<template>
 ...
 : 'border-border bg-surface hover:border-brand/40 hover:shadow-sm hover:scale-[1.01]'"
 @click="$emit('update:modelValue', tpl.id)"
 >
 <!-- Mock visual miniature preview area -->
 <div class="h-20 w-full bg-background/60 border-b border-border p-2 flex gap-1 overflow-hidden transition-all duration-300 group-hover:bg-surface-hover/40">
 ...
 <div class="h-1 w-full bg-surface-hover rounded-sm"></div>
 <div class="h-1 w-full bg-surface-hover rounded-sm"></div>
 ...
 <div class="flex items-center justify-between border-b border-border pb-1 mb-0.5">
 <div class="h-2 w-1/3 bg-slate-400 rounded-sm"></div>
 <div class="h-1 w-1/4 bg-slate-300 rounded-sm"></div>
 </div>
 <div class="h-1 w-1/4 bg-slate-300 rounded-sm"></div>
 <div class="h-1 w-full bg-surface-hover rounded-sm"></div>
 <div class="h-1 w-5/6 bg-surface-hover rounded-sm"></div>
 ...
 <div class="h-1 w-full bg-surface-hover rounded-sm mt-1"></div>
 <div class="h-1 w-full bg-surface-hover rounded-sm"></div>
 <div class="h-1 w-2/3 bg-surface-hover rounded-sm"></div>
 </div>
 </template>
 ...
 <div class="bg-surface-elevated text-white p-1 px-1.5 flex items-center justify-between rounded-t-sm shrink-0">
 <div class="h-1.5 w-1/4 bg-slate-300 rounded-sm"></div>
 <div class="h-1 w-1/5 bg-slate-400 rounded-sm"></div>
 </div>
 <div class="p-1 flex-1 flex flex-col gap-1 bg-background shrink-0">
 ...
 <span class="text-xs font-bold text-txt-primary" :class="{ 'text-brand': modelValue === tpl.id }">{{ tpl.label }}</span>
 ...
 <p class="text-[10px] text-txt-muted line-clamp-2 leading-snug">{{ tpl.description }}</p>
 </div>
 </button>
 </div>
 </div>
...
const atsBadgeClass = computed(() => {
 ...
 return 'bg-surface-hover text-txt-secondary'
})
</script>
