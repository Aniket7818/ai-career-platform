  ...
  <div class="intelligence-panel space-y-4">
    ...
    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
      <div class="flex items-center gap-2">
        ...
        <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700">Resume Intelligence</h3>
      </div>
      <span class="rounded-full bg-emerald-50 text-emerald-700 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider">Free</span>
    </div>

    <!-- ═══ 1. Health Score HERO ═══ -->
    <div class="flex flex-col items-center justify-center p-6 text-center bg-white rounded-2xl shadow-[0_4px_20px_rgba(15,23,42,0.03)] border border-slate-100/50 relative overflow-hidden group">
      ...
      <div class="relative flex items-center justify-center size-28 mb-4 cursor-pointer select-none" @click="toggleSection('health')">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center z-10 leading-none">
          <span class="text-3xl font-black text-slate-900 tracking-tight" :class="{ 'animate-score-pulse': scoreJustChanged }">{{ animatedScore }}</span>
          <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Health</span>
        </div>
      </div>

      <div class="space-y-1 z-10">
        <h4 class="text-base font-bold text-slate-900 leading-tight">{{ healthLabel }}</h4>
        <div class="flex items-center justify-center gap-1.5 mt-1">
          <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-slate-50 text-slate-600 border border-slate-100">{{ experienceLevel.label }}</span>
      ...
      <Transition name="slide">
        <div v-if="activeSection === 'health'" class="w-full mt-4 pt-4 border-t border-slate-100 space-y-3 text-left">
          <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">Score Breakdown</p>
          <div v-for="s in fullScoreBreakdown" :key="s.name" class="text-[11px] space-y-1">
            ...
            <p class="text-[9px] text-slate-500 leading-normal">{{ s.reason }}</p>
          </div>
        </div>
      </Transition>
    </div>
    ...
    <div class="intel-card group" :class="{ 'is-expanded': activeSection === 'coach' }">
      <button type="button" class="w-full text-left focus:outline-none" @click="toggleSection('coach')" :aria-expanded="activeSection === 'coach'">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-bold text-slate-700">Resume Coach</span>
          </div>
      ...
      <Transition name="slide">
        <div v-if="activeSection === 'coach'" class="mt-3 border-t border-slate-100 pt-3 space-y-3">
          <template v-if="canUseResumeCoach">
            <div class="rounded-xl bg-gradient-to-br from-brand/5 to-purple-50 p-4 border border-brand/10">
              <p class="text-sm font-semibold text-ink leading-relaxed">Your resume is stronger than <span class="text-brand font-bold">{{ coachPercentile }}%</span> of {{ experienceLevel.label }} resumes.</p>
              <div class="mt-3 flex items-start gap-2">
                <span class="shrink-0 mt-0.5 text-brand">💡</span>
                <div>
                  <p class="text-[11px] font-bold text-slate-700">{{ coachRecommendation.title }}</p>
                  <p class="text-[10px] text-slate-500 mt-0.5">{{ coachRecommendation.detail }}</p>
                  <p class="text-[10px] font-bold text-brand mt-1">Estimated improvement: {{ coachRecommendation.impact }}</p>
                </div>
              </div>
            </div>
            <button type="button" class="w-full rounded-lg bg-slate-50 border border-slate-100 px-3 py-2 text-[10px] font-semibold text-slate-400 flex items-center justify-center gap-1.5 cursor-not-allowed" title="AI coaching coming soon">
              ...
              Get AI Coaching
              <span class="rounded bg-slate-100 px-1 py-0.5 text-[8px] font-bold text-slate-500">COMING SOON</span>
            </button>
          </template>
          <template v-else>
            <PremiumFeatureCard
    ...
    <div class="intel-card group" :class="{ 'is-expanded': activeSection === 'checklist' }">
      <button type="button" class="w-full text-left focus:outline-none" @click="toggleSection('checklist')" :aria-expanded="activeSection === 'checklist'">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <svg class="size-4 text-slate-400 group-hover:text-brand transition-colors duration-250" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>
            <span class="text-xs font-bold text-slate-700">Checklist</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="text-[10px] font-semibold text-slate-500">{{ checklistDone }}/{{ checklist.length }} Completed</span>
      ...
      <Transition name="slide">
        <div v-if="activeSection === 'checklist'" class="mt-3 space-y-2 border-t border-slate-100 pt-3">
          <p class="text-[10px] font-medium text-slate-500 mb-2">
          ...
          <div v-for="(item, i) in sortedChecklist" :key="i" class="rounded-lg p-2 text-[11px] transition-colors" :class="item.done ? 'bg-transparent opacity-60' : item.warning ? 'bg-amber-50/50' : 'bg-red-50/50'">
            <div class="flex items-start gap-2">
              ...
              <div>
                <p class="font-semibold" :class="item.done ? 'text-slate-500 line-through' : 'text-slate-800'">{{ item.label }}</p>
                <div v-if="!item.done" class="mt-1 space-y-0.5">
                  <p class="text-slate-600">{{ item.why }}</p>
    ...
    <div class="intel-card group" :class="{ 'is-expanded': activeSection === 'ats' }">
      <button type="button" class="w-full text-left focus:outline-none" @click="toggleSection('ats')" :aria-expanded="activeSection === 'ats'">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span v-else class="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-bold text-slate-500 flex items-center gap-0.5">
      ...
      <Transition name="slide">
        <div v-if="activeSection === 'ats'" class="mt-3 border-t border-slate-100 pt-3">
              ...
              <div v-for="rule in atsAnalysis.rules" :key="rule.id" class="rounded-lg p-2 text-[11px] cursor-pointer transition-colors" :class="rule.passed ? 'bg-emerald-50/50 hover:bg-emerald-50' : rule.category === 'critical' ? 'bg-red-50/50 hover:bg-red-50' : 'bg-amber-50/50 hover:bg-amber-50'" @click="toggleAtsRule(rule.id)">
                <div class="flex items-start justify-between gap-2">
                  <div class="flex items-start gap-2">
                    ...
                    <p class="font-semibold text-slate-800">{{ rule.title }}</p>
                  </div>
                ...
                <Transition name="slide-fast">
                  <div v-if="expandedAtsRules[rule.id]" class="mt-2 pl-5 space-y-1">
                    <p class="text-slate-600"><span class="font-medium text-slate-700">Why it matters:</span> {{ rule.description }}</p>
                    <p v-if="!rule.passed" class="text-slate-600"><span class="font-medium text-slate-700">Impact:</span> {{ rule.impact }}</p>
    ...
    <div class="intel-card group" :class="{ 'is-expanded': activeSection === 'keywords' }">
      <button type="button" class="w-full text-left focus:outline-none" @click="toggleSection('keywords')" :aria-expanded="activeSection === 'keywords'">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span v-else-if="!canUseKeywords" class="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-bold text-slate-500 flex items-center gap-0.5">
              <svg class="size-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
              Locked
            </span>
          </div>
      ...
      <Transition name="slide">
        <div v-if="activeSection === 'keywords'" class="mt-3 border-t border-slate-100 pt-3">
          <template v-if="canUseKeywords">
            <div v-if="!targetRole" class="rounded-lg bg-slate-50 p-3 text-center border border-slate-100">
              ...
              <p class="text-xs font-semibold text-slate-700">No Target Role Selected</p>
              <p class="text-[10px] text-slate-500 mt-1">Select a target job role in the form above to see ATS keyword match analysis.</p>
            </div>
            <div v-else class="space-y-3">
              <div>
                <p class="text-[10px] text-slate-500">Analysis based on: <span class="font-bold text-brand">{{ targetRole }}</span></p>
              ...
              <div class="text-[11px]">
                <div class="flex justify-between mb-1">
                  <span class="font-medium text-slate-700">Coverage</span>
                  <span class="font-semibold text-ink">{{ keywordAnalysis.coverage }}%</span>
                </div>
                <div class="h-1.5 w-full rounded-full bg-slate-100 overflow-hidden">
    ...
    <div class="intel-card group" :class="{ 'is-expanded': activeSection === 'stats' }">
      <button type="button" class="w-full text-left focus:outline-none" @click="toggleSection('stats')" :aria-expanded="activeSection === 'stats'">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-bold text-slate-700">Statistics</span>
          </div>
      ...
      <Transition name="slide">
        <div v-if="activeSection === 'stats'" class="mt-3 border-t border-slate-100 pt-3">
          <div class="grid grid-cols-2 gap-2">
            <div v-for="(val, key) in statsDisplay" :key="key" class="flex flex-col rounded-xl bg-slate-50/50 border border-slate-100/60 p-3 relative overflow-hidden group">
              <!-- Decorative subtle background element -->
              <div class="absolute -right-2 -bottom-2 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity text-slate-900 z-0">
              ...
              <span class="text-xs font-extrabold text-slate-800 z-10">{{ val }}</span>
            </div>
          </div>
        </div>
      </Transition>
    ...
    <div class="intel-card group" :class="{ 'is-expanded': activeSection === 'level' }">
      <button type="button" class="w-full text-left focus:outline-none" @click="toggleSection('level')" :aria-expanded="activeSection === 'level'">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-bold text-slate-700">Level Profile</span>
          </div>
      ...
      <Transition name="slide">
        <div v-if="activeSection === 'level'" class="mt-3 border-t border-slate-100 pt-3 space-y-3">
          <div class="grid grid-cols-2 gap-2 text-[11px]">
            <div>
              <p class="text-slate-400 text-[10px]">Career Level</p>
              <p class="font-bold text-slate-700">{{ experienceLevel.label }}</p>
            </div>
            <div>
              <p class="text-slate-400 text-[10px]">Experience</p>
              <p class="font-bold text-slate-700">{{ experienceLevel.years }} Years</p>
            </div>
            <div>
              <p class="text-slate-400 text-[10px]">Rec. Length</p>
              <p class="font-bold text-slate-700">{{ experienceLevel.pages }} page{{ experienceLevel.pages > 1 ? 's' : '' }}</p>
            </div>
            <div>
...
onMounted(() => {
  ...
  if (saved) activeSection.value = saved
...
const toggleSection = (section) => {
  ...
  if (activeSection.value) {
    localStorage.setItem('careerai_intel_accordion', activeSection.value)
  } else {
    localStorage.removeItem('careerai_intel_accordion')
  }
}
...
<style scoped>
.intel-card {
  @apply rounded-xl border border-slate-100 bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.01),0_1px_2px_rgba(0,0,0,0.02)] transition-all duration-200;
}
.intel-card.is-expanded {
  @apply shadow-[0_10px_30px_rgba(15,23,42,0.04)] border-brand/10;
}
.intel-card:hover:not(.is-expanded) {
  @apply border-slate-200/80 shadow-[0_2px_8px_rgba(0,0,0,0.02)] -translate-y-[0.5px];
}
