<template>
 ...
 <h3 class="text-xs font-bold uppercase tracking-wider text-txt-secondary">Resume Intelligence</h3>
 </div>
 <span class="rounded-full bg-emerald-50 text-emerald-700 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider">Free</span>
 </div>

 <!-- ═══ 1. Health Score HERO ═══ -->
 <div class="flex flex-col items-center justify-center p-6 text-center bg-surface rounded-2xl shadow-[0_4px_20px_rgba(15,23,42,0.03)] border border-border/50 relative overflow-hidden group">
 <!-- Glow effect behind the ring -->
 <div class="absolute -inset-10 rounded-full bg-brand/5 blur-3xl group-hover:bg-brand/10 transition-all duration-300 pointer-events-none"></div>
 ...
 <span class="text-3xl font-black text-txt-primary tracking-tight" :class="{ 'animate-score-pulse': scoreJustChanged }">{{ animatedScore }}</span>
 <span class="text-[9px] font-bold text-txt-disabled uppercase tracking-widest mt-1">Health</span>
 </div>
 </div>

 <div class="space-y-1 z-10">
 <h4 class="text-base font-bold text-txt-primary leading-tight">{{ healthLabel }}</h4>
 <div class="flex items-center justify-center gap-1.5 mt-1">
 <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-background text-txt-secondary border border-border ">{{ experienceLevel.label }}</span>
 <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full" :class="[
 healthScore >= 80 ? 'bg-emerald-50 text-emerald-700 border border-emerald-100/50 ' :
 healthScore >= 50 ? 'bg-amber-50 text-amber-700 border border-amber-100/50 ' : 'bg-rose-50 text-rose-700 border border-rose-100/50 '
 ]">
 ...
 <div v-if="activeSection['health']" class="w-full mt-4 pt-4 border-t border-border space-y-3 text-left">
 <p class="text-[10px] font-bold uppercase tracking-wider text-txt-disabled mb-2">Score Breakdown</p>
 <div v-for="s in fullScoreBreakdown" :key="s.name" class="text-[11px] space-y-1">
 <div class="flex justify-between">
 <span class="font-medium text-txt-secondary">{{ s.name }}</span>
 ...
 <p class="text-[9px] text-txt-muted leading-normal">{{ s.reason }}</p>
 </div>
 </div>
 </Transition>
 </div>
 ...
 <div class="flex items-center gap-2">
 <svg class="size-4 text-txt-disabled group-hover:text-brand transition-colors duration-250" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
 <span class="text-xs font-bold text-txt-secondary">Resume Coach</span>
 </div>
 <svg class="size-4 text-txt-disabled transition-transform duration-250 ease-out" :class="activeSection['coach'] && 'rotate-180'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </div>
 </button>
 <Transition name="slide">
 <div v-if="activeSection['coach']" class="mt-3 border-t border-border pt-3 space-y-3">
 <template v-if="canUseResumeCoach">
 <div class="rounded-xl bg-gradient-to-br from-brand/5 to-purple-50 p-4 border border-brand/10 ">
 <p class="text-sm font-semibold text-txt-primary leading-relaxed">Your resume is stronger than <span class="text-brand font-bold">{{ coachPercentile }}%</span> of {{ experienceLevel.label }} resumes.</p>
 <div class="mt-3 flex items-start gap-2">
 <span class="shrink-0 mt-0.5 text-brand">💡</span>
 <div>
 <p class="text-[11px] font-bold text-txt-secondary">{{ coachRecommendation.title }}</p>
 <p class="text-[10px] text-txt-muted mt-0.5">{{ coachRecommendation.detail }}</p>
 <p class="text-[10px] font-bold text-brand mt-1">Estimated improvement: {{ coachRecommendation.impact }}</p>
 </div>
 </div>
 </div>
 <button type="button" class="w-full rounded-lg bg-surface-elevated/50 border border-border/50 px-3 py-2 text-[10px] font-semibold text-txt-disabled flex items-center justify-center gap-1.5 cursor-not-allowed" title="AI coaching coming soon">
 ...
 Get AI Coaching
 <span class="rounded bg-surface-hover px-1 py-0.5 text-[8px] font-bold text-txt-muted">COMING SOON</span>
 </button>
 </template>
 <template v-else>
 <PremiumFeatureCard
 ...
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-2">
 <svg class="size-4 text-txt-disabled group-hover:text-brand transition-colors duration-250" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a4 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>
 <span class="text-xs font-bold text-txt-secondary">Checklist</span>
 </div>
 <div class="flex items-center gap-2">
 <span class="text-[10px] font-semibold text-txt-muted">{{ checklistDone }}/{{ checklist.length }} Completed</span>
 <svg class="size-4 text-txt-disabled transition-transform duration-250 ease-out" :class="activeSection['checklist'] && 'rotate-180'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </div>
 </div>
 </button>
 <Transition name="slide">
 <div v-if="activeSection['checklist']" class="mt-3 space-y-2 border-t border-border pt-3">
 <p class="text-[10px] font-medium text-txt-muted mb-2">
 ...
 <div v-for="(item, i) in sortedChecklist" :key="i" class="rounded-lg p-2 text-[11px] transition-colors" :class="item.done ? 'bg-transparent opacity-60' : item.warning ? 'bg-amber-50/50 ' : 'bg-red-50/50 '">
 ...
 <div>
 <p class="font-semibold" :class="item.done ? 'text-txt-muted line-through' : 'text-txt-primary '">{{ item.label }}</p>
 <div v-if="!item.done" class="mt-1 space-y-0.5">
 <p class="text-txt-secondary ">{{ item.why }}</p>
 ...
 <span class="text-xs font-bold text-txt-secondary">ATS Score</span>
 <span v-if="canUseATS" class="rounded-full px-1.5 py-0.5 text-[9px] font-bold" :class="atsScoreBadge">{{ atsAnalysis.score }}%</span>
 <span v-else class="rounded-full bg-surface-hover px-1.5 py-0.5 text-[9px] font-bold text-txt-muted flex items-center gap-0.5">
 ...
 <svg class="size-4 text-txt-disabled transition-transform duration-250 ease-out" :class="activeSection['ats'] && 'rotate-180'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </div>
 </button>
 <Transition name="slide">
 <div v-if="activeSection['ats']" class="mt-3 border-t border-border pt-3">
 <template v-if="canUseATS">
 <div class="space-y-2 max-h-[350px] overflow-y-auto custom-scrollbar w-full">
 <div v-for="rule in atsAnalysis.rules" :key="rule.id" class="rounded-lg p-2 text-[11px] cursor-pointer transition-colors" :class="rule.passed ? 'bg-emerald-50/50 hover:bg-emerald-50 ' : rule.category === 'critical' ? 'bg-red-50/50 hover:bg-red-50 ' : 'bg-amber-50/50 hover:bg-amber-50 '" @click="toggleAtsRule(rule.id)">
 ...
 <p class="font-semibold text-txt-primary ">{{ rule.title }}</p>
 </div>
 <svg class="size-3 text-txt-disabled transition-transform" :class="expandedAtsRules[rule.id] && 'rotate-180'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </div>
 
 <Transition name="slide-fast">
 <div v-if="expandedAtsRules[rule.id]" class="mt-2 pl-5 space-y-1">
 <p class="text-txt-secondary "><span class="font-medium text-txt-secondary">Why it matters:</span> {{ rule.description }}</p>
 <p v-if="!rule.passed" class="text-txt-secondary "><span class="font-medium text-txt-secondary">Impact:</span> {{ rule.impact }}</p>
 ...
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-2">
 <svg class="size-4 text-txt-disabled group-hover:text-brand transition-colors duration-250" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
 <span class="text-xs font-bold text-txt-secondary">Keywords</span>
 ...
 <span v-else-if="!canUseKeywords" class="rounded-full bg-surface-hover px-1.5 py-0.5 text-[9px] font-bold text-txt-muted flex items-center gap-0.5">
 <svg class="size-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
 Locked
 </span>
 </div>
 <svg class="size-4 text-txt-disabled transition-transform duration-250 ease-out" :class="activeSection['keywords'] && 'rotate-180'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </div>
 </button>
 <Transition name="slide">
 <div v-if="activeSection['keywords']" class="mt-3 border-t border-border pt-3">
 <template v-if="canUseKeywords">
 <div v-if="!targetRole" class="rounded-lg bg-surface-elevated/50 p-3 text-center border border-border/50">
 <svg class="size-6 text-txt-disabled mx-auto mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
 <p class="text-xs font-semibold text-txt-secondary">No Target Role Selected</p>
 <p class="text-[10px] text-txt-muted mt-1">Select a target job role in the form above to see ATS keyword match analysis.</p>
 </div>
 <div v-else class="space-y-3">
 <div>
 <p class="text-[10px] text-txt-muted">Analysis based on: <span class="font-bold text-brand">{{ targetRole }}</span></p>
 <p class="text-[9px] text-txt-disabled mt-0.5">This role is only used for keyword analysis and is not added to your resume.</p>
 </div>
 
 <div class="text-[11px]">
 <div class="flex justify-between mb-1">
 <span class="font-medium text-txt-secondary">Coverage</span>
 <span class="font-semibold text-txt-primary ">{{ keywordAnalysis.coverage }}%</span>
 </div>
 <div class="h-1.5 w-full rounded-full bg-surface-hover overflow-hidden">
 ...
 <span v-for="kw in keywordAnalysis.detected" :key="kw" class="rounded bg-emerald-50 px-1.5 py-0.5 text-[10px] font-medium text-emerald-700 border border-emerald-100 ">{{ kw }}</span>
 <span v-if="keywordAnalysis.detected.length === 0" class="text-[10px] text-txt-disabled italic">No exact matches yet</span>
 </div>
 </div>
 ...
 <div class="flex flex-wrap gap-1">
 <span v-for="kw in keywordAnalysis.missing.slice(0, 15)" :key="kw" class="rounded bg-amber-50 px-1.5 py-0.5 text-[10px] font-medium text-amber-700 border border-amber-100 ">{{ kw }}</span>
 <span v-if="keywordAnalysis.missing.length > 15" class="text-[10px] text-txt-disabled">+{{ keywordAnalysis.missing.length - 15 }} more</span>
 </div>
 </div>
 </div>
 </template>
 ...
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-2">
 <svg class="size-4 text-txt-disabled group-hover:text-brand transition-colors duration-250" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 20V10M12 20V4M6 20v-6"/></svg>
 <span class="text-xs font-bold text-txt-secondary">Statistics</span>
 </div>
 <svg class="size-4 text-txt-disabled transition-transform duration-250 ease-out" :class="activeSection['stats'] && 'rotate-180'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </div>
 </button>
 <Transition name="slide">
 <div v-if="activeSection['stats']" class="mt-3 border-t border-border pt-3">
 <div class="grid grid-cols-2 gap-2">
 <div v-for="(val, key) in statsDisplay" :key="key" class="flex flex-col rounded-xl bg-background/50 border border-border/60 p-3 relative overflow-hidden group">
 <!-- Decorative subtle background element -->
 <div class="absolute -right-2 -bottom-2 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity text-txt-primary z-0">
 ...
 <span class="text-[9px] font-bold uppercase tracking-widest text-txt-disabled mb-1 z-10">{{ key }}</span>
 <span class="text-xs font-extrabold text-txt-primary z-10">{{ val }}</span>
 </div>
 </div>
 </div>
 </Transition>
 ...
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-2">
 <svg class="size-4 text-txt-disabled group-hover:text-brand transition-colors duration-250" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="8.5" cy="7" r="4"/><path d="M20 8v6M23 11h-6"/></svg>
 <span class="text-xs font-bold text-txt-secondary">Level Profile</span>
 </div>
 ...
 <svg class="size-4 text-txt-disabled transition-transform duration-250 ease-out" :class="activeSection['level'] && 'rotate-180'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </div>
 </div>
 </button>
 <Transition name="slide">
 <div v-if="activeSection['level']" class="mt-3 border-t border-border pt-3 space-y-3">
 <div class="grid grid-cols-2 gap-2 text-[11px]">
 <div>
 <p class="text-txt-disabled text-[10px]">Career Level</p>
 <p class="font-bold text-txt-secondary">{{ experienceLevel.label }}</p>
 </div>
 <div>
 <p class="text-txt-disabled text-[10px]">Experience</p>
 <p class="font-bold text-txt-secondary">{{ experienceLevel.years }} Years</p>
 </div>
 <div>
 <p class="text-txt-disabled text-[10px]">Rec. Length</p>
 <p class="font-bold text-txt-secondary">{{ experienceLevel.pages }} page{{ experienceLevel.pages > 1 ? 's' : '' }}</p>
 </div>
 <div>
 <p class="text-txt-disabled text-[10px]">Readiness</p>
 <p class="font-bold text-emerald-600 ">{{ experienceLevel.readiness }}</p>
 </div>
 </div>
 <div>
 <p class="text-[10px] text-txt-muted mb-1.5">Recommended Focus Areas:</p>
 <div class="flex flex-wrap gap-1">
...
<style scoped>
.intel-card {
 @apply rounded-xl border border-border bg-surface p-4 shadow-[0_1px_3px_rgba(0,0,0,0.01),0_1px_2px_rgba(0,0,0,0.02)] transition-all duration-200;
}
.intel-card.is-expanded {
 @apply shadow-[0_10px_30px_rgba(15,23,42,0.04)] border-brand/10 ;
}
.intel-card:hover:not(.is-expanded) {
 @apply border-border/80 shadow-[0_2px_8px_rgba(0,0,0,0.02)] -translate-y-[0.5px];
}
