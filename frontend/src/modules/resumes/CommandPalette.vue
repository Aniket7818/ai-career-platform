<template>
 ...
 <!-- Backdrop -->
 <div class="fixed inset-0 bg-surface/40 backdrop-blur-sm" @click="close"></div>
 
 <!-- Palette -->
 <div class="relative w-full max-w-lg bg-surface rounded-2xl shadow-2xl overflow-hidden flex flex-col" @click.stop>
 <div class="flex items-center px-4 border-b border-border">
 <svg class="size-5 text-txt-disabled" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
 <input 
 ref="searchInput"
 v-model="query"
 type="text"
 class="flex-1 bg-transparent border-none focus:ring-0 text-base py-4 px-3 outline-none text-txt-primary placeholder:text-txt-disabled"
 ...
 />
 <span class="text-[10px] font-bold text-txt-disabled bg-surface-hover px-1.5 py-0.5 rounded border border-border uppercase tracking-widest">ESC</span>
 </div>
 ...
 :class="selectedIndex === index ? 'bg-brand/5 text-brand' : 'text-txt-secondary hover:bg-background'"
 @click="execute(cmd)"
 @mouseenter="selectedIndex = index"
 >
 <div class="flex items-center gap-3">
 <span class="text-sm" :class="selectedIndex === index ? 'text-brand' : 'text-txt-disabled'" v-html="cmd.icon"></span>
 <span class="font-semibold text-sm">{{ cmd.label }}</span>
 </div>
 <span class="text-[10px] font-medium text-txt-disabled">{{ cmd.category }}</span>
 </div>
 </template>
 <div v-else class="px-4 py-8 text-center">
 <p class="text-sm text-txt-muted font-medium">No commands found for "{{ query }}"</p>
 </div>
 </div>
 
 <div class="bg-background border-t border-border px-4 py-2 flex items-center justify-between text-[10px] font-medium text-txt-muted">
 <div class="flex gap-4">
 <span class="flex items-center gap-1"><kbd class="bg-surface px-1.5 py-0.5 rounded border border-border font-sans shadow-sm">↑</kbd><kbd class="bg-surface px-1.5 py-0.5 rounded border border-border font-sans shadow-sm">↓</kbd> to navigate</span>
 <span class="flex items-center gap-1"><kbd class="bg-surface px-1.5 py-0.5 rounded border border-border font-sans shadow-sm">Enter</kbd> to select</span>
 </div>
 </div>
 </div>
 </div>
