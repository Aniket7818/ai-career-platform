<template>
 <form
 class="flex flex-col lg:flex-row min-h-screen lg:h-screen w-full lg:overflow-hidden bg-background"
 ...
 <aside 
 v-if="!isMobile"
 class="left-sidebar relative flex shrink-0 flex-col border-b lg:border-b-0 lg:border-r border-border bg-surface transition-all duration-300 ease-in-out"
 :class="leftSidebarCollapsed ? '!w-0 !border-r-0 overflow-hidden opacity-0' : ''"
 :style="{ '--lg-width': leftSidebarCollapsed ? '0px' : `${leftWidth}px` }"
 >
 <div class="flex items-center gap-3 border-b border-border p-5 shrink-0">
 <RouterLink to="/resumes" class="inline-flex items-center gap-2 text-sm font-semibold text-txt-muted transition hover:text-txt-primary">
 ...
 <button type="button" class="w-full flex items-center justify-between px-5 py-4 text-left transition hover:bg-background/50" @click="sidebarState.template = !sidebarState.template">
 <span class="text-xs font-bold uppercase tracking-wider text-txt-muted">Template</span>
 <svg class="size-4 text-txt-disabled transition-transform duration-200" :class="sidebarState.template ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </button>
 ...
 <!-- Appearance Section -->
 <div class="border-b border-border">
 <button type="button" class="w-full flex items-center justify-between px-5 py-4 text-left transition hover:bg-background/50" @click="sidebarState.appearance = !sidebarState.appearance">
 <span class="text-xs font-bold uppercase tracking-wider text-txt-muted">Settings</span>
 <svg class="size-4 text-txt-disabled transition-transform duration-200" :class="sidebarState.appearance ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </button>
 <Transition name="collapse">
 ...
 <!-- Customize Section -->
 <div class="border-b border-border">
 <button type="button" class="w-full flex items-center justify-between px-5 py-4 text-left transition hover:bg-background/50" @click="sidebarState.customize = !sidebarState.customize">
 <span class="text-xs font-bold uppercase tracking-wider text-txt-muted">Reorder & Layout</span>
 <svg class="size-4 text-txt-disabled transition-transform duration-200" :class="sidebarState.customize ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
 </button>
 <Transition name="collapse">
 ...
 >
 <div class="w-[2px] h-full bg-surface-hover group-hover:bg-brand group-active:bg-brand mx-auto transition-colors duration-200"></div>
 </div>
 </aside>
 ...
 <div class="mb-8 flex flex-col gap-6 md:flex-row md:items-center md:justify-between border-b border-border pb-6">
 <div class="flex items-center gap-3">
 <!-- Sidebar Toggle Button for Desktop -->
 <button 
 v-if="!isMobile"
 type="button" 
 class="p-2 text-txt-disabled hover:text-txt-secondary hover:bg-surface-hover rounded-lg transition-colors duration-200 shrink-0"
 ...
 <p class="text-xs text-txt-muted mt-1">Let’s craft a world-class resume today.</p>
 </div>
 </div>
 ...
 <div class="flex flex-col">
 <span class="font-bold uppercase tracking-wider text-txt-disabled text-[9px] mb-0.5">Resume Health</span>
 <span class="font-extrabold" :class="'text-' + healthColor + '-600'">{{ healthScore }}%</span>
 </div>
 <div class="h-6 w-px bg-surface-hover"></div>
 <div class="flex flex-col">
 <span class="font-bold uppercase tracking-wider text-txt-disabled text-[9px] mb-0.5">ATS Score</span>
 <span class="font-extrabold text-txt-primary">{{ atsAnalysis.score }}</span>
 </div>
 <div class="h-6 w-px bg-surface-hover"></div>
 <div class="flex flex-col">
 <span class="font-bold uppercase tracking-wider text-txt-disabled text-[9px] mb-0.5">Last Saved</span>
 <span class="font-extrabold text-txt-primary">
 ...
 <div class="h-6 w-px bg-surface-hover hidden sm:block"></div>
 <div class="flex flex-col hidden sm:flex">
 <span class="font-bold uppercase tracking-wider text-txt-disabled text-[9px] mb-0.5">Completion</span>
 <span class="font-extrabold text-txt-primary">{{ completionScore }}%</span>
 </div>
 </div>
 </div>

 <!-- Resume meta: title + status + target role -->
 <div class="rounded-2xl border border-border bg-surface p-5 shadow-panel sm:p-6 space-y-4">
 ...
 <div>
 <p class="text-sm font-semibold text-txt-primary">{{ t('resumes.fillFromProfile') }}</p>
 <p class="text-xs text-txt-muted">{{ t('resumes.fillFromProfileHint') }}</p>
 </div>
 <button
 type="button"
 class="rounded-xl border border-brand/30 bg-surface px-4 py-2 text-sm font-semibold text-brand transition hover:bg-brand/10"
 ...
 :class="openSections[section.key] ? 'bg-surface shadow-panel ring-1 ring-black/5' : 'bg-transparent hover:bg-surface/60'"
 >
 <!-- Section header (click to toggle) -->
 <button
 type="button"
 class="flex w-full items-center justify-between gap-3 p-5 sm:p-6 text-left transition hover:bg-background/70"
 ...
 <span class="text-sm font-bold text-txt-primary flex items-center">
 {{ section.label }}
 <span v-if="getSectionCount(section.key) > 0" class="ml-2 rounded bg-surface-hover px-1.5 py-0.5 text-[10px] font-semibold text-txt-muted">{{ getSectionCount(section.key) }}</span>
 </span>
 <!-- Show hint if section is empty and expanded -->
 <span v-if="openSections[section.key] && getSectionCount(section.key) === 0 && section.hint" class="text-[11px] text-txt-muted mt-0.5 max-w-[240px] leading-snug animate-fade-in">
 {{ section.hint }}
 </span>
 </div>
 </div>
 ...
 <svg
 class="size-4 text-txt-disabled transition-transform"
 ...
 <div class="mt-2 text-xs text-txt-muted italic" v-else>
 ...
 <p class="mt-1 text-xs text-txt-disabled">Separate skills with commas for best ATS results.</p>
 </div>
 </template>
 ...
 <span class="text-xs text-txt-disabled flex items-center gap-1">
 ...
 <aside 
 v-if="!isMobile"
 class="right-sidebar relative flex shrink-0 flex-col border-t lg:border-t-0 lg:border-l border-border bg-surface-hover/30"
 ...
 <div class="flex items-center justify-between border-b border-border/60 bg-surface p-4">
 <div class="flex items-center gap-2">
 <h2 class="text-sm font-bold text-txt-primary">{{ t('resumes.preview') }}</h2>
 ...
 <div class="flex items-center gap-1 bg-surface border border-border/80 rounded-full p-1.5 shadow-[0_2px_8px_rgba(15,23,42,0.04)] backdrop-blur-sm">
 <button type="button" class="p-1.5 text-txt-muted hover:bg-surface-hover rounded-full transition-colors" @click="zoomOut" title="Zoom Out">
 ...
 <button type="button" class="text-[10px] font-bold text-txt-secondary w-10 text-center select-none hover:bg-surface-hover rounded-full transition cursor-pointer" @click="previewZoom = 100" title="Reset to 100%">
 {{ Math.round(previewZoom) }}%
 </button>
 <button type="button" class="p-1.5 text-txt-muted hover:bg-surface-hover rounded-full transition-colors" @click="zoomIn" title="Zoom In">
 ...
 <div class="w-px h-4 bg-surface-hover mx-0.5"></div>
 <button type="button" class="px-2 py-1 text-txt-muted hover:bg-surface-hover rounded-full transition text-[9px] font-bold uppercase tracking-wider flex items-center gap-0.5" @click="fitToWidth" title="Fit Width">
 ...
 <button type="button" class="px-2 py-1 text-txt-muted hover:bg-surface-hover rounded-full transition text-[9px] font-bold uppercase tracking-wider flex items-center gap-0.5" @click="fitToPage" title="Fit Page">
 ...
 <div class="w-px h-4 bg-surface-hover mx-0.5"></div>
 <button type="button" class="p-1.5 text-txt-muted hover:bg-surface-hover rounded-full transition-colors" @click="showFullPreview = true" title="Fullscreen">
 ...
 <button
 type="button"
 class="rounded-xl bg-surface/10 px-4 py-2 text-sm font-semibold text-white transition hover:bg-surface/20"
 ...
 <div class="flex items-center justify-between border-b border-border bg-surface px-4 py-3 sticky top-0 z-10">
 <h2 class="text-sm font-bold text-txt-primary">Preview</h2>
 <div class="flex items-center gap-2">
 <!-- Mobile Toolbar -->
 <div class="flex items-center gap-1 bg-background border border-border rounded-lg p-1">
 <button type="button" class="p-1 text-txt-muted hover:bg-surface-hover rounded transition" @click="zoomOut">
 ...
 <button type="button" class="text-[10px] font-bold text-txt-secondary w-9 text-center select-none hover:bg-surface-hover rounded transition" @click="previewZoom = 100">{{ Math.round(previewZoom) }}%</button>
 <button type="button" class="p-1 text-txt-muted hover:bg-surface-hover rounded transition" @click="zoomIn">
 ...
 <div class="flex-1 overflow-auto bg-surface-hover p-4">
 <ResumePreview
 ...
 <div v-if="isMobile && activeTab === 'insights'" class="flex-1 overflow-y-auto bg-background p-4 space-y-4">
 <div class="flex items-center gap-2 mb-2">
 <h2 class="text-lg font-bold text-txt-primary">Resume Intelligence</h2>
 ...
 <div v-if="isMobile && activeTab === 'more'" class="flex-1 overflow-y-auto bg-background p-5 space-y-5">
 <div class="flex items-center justify-between border-b border-border/50 pb-3">
 <h2 class="text-base font-bold text-txt-primary">Settings</h2>
 <RouterLink to="/resumes" class="text-xs font-semibold text-txt-muted hover:text-txt-primary flex items-center gap-1">
 ...
 <div class="flex items-center gap-1 overflow-x-auto scrollbar-none bg-surface-hover p-1 rounded-xl shrink-0">
 <button 
 ...
 :class="activeSettingsCategory === cat ? 'bg-surface text-txt-primary shadow-sm' : 'text-txt-muted hover:text-txt-primary'"
 ...
 <p class="text-xs font-bold text-txt-disabled uppercase tracking-wider">Templates</p>
 <TemplatePicker v-model="model.template_id" :content="model.content" />
 </div>
 <div v-else-if="activeSettingsCategory === 'content'" class="space-y-3">
 <p class="text-xs font-bold text-txt-disabled uppercase tracking-wider">Layout & Reorder</p>
 <CustomizePanel v-model:order="model.content.sectionOrder" v-model:visibility="model.content.sectionVisibility" />
 </div>
 <div v-else-if="activeSettingsCategory === 'appearance'" class="space-y-3">
 <p class="text-xs font-bold text-txt-disabled uppercase tracking-wider">Appearance</p>
 <AppearancePanel v-model="model.content.appearance" />
 </div>
 <div v-else-if="activeSettingsCategory === 'export'" class="space-y-3">
 <p class="text-xs font-bold text-txt-disabled uppercase tracking-wider">Export Document</p>
 <div class="rounded-2xl border border-border bg-surface p-5 shadow-sm space-y-4">
 <p class="text-xs text-txt-muted leading-normal">Ready to download? Your resume is prepared for instant export to a clean, professionally pagination-validated PDF.</p>
 <button 
 type="button" 
 ...
 <div
 v-if="isExpanded"
 class="absolute z-50 rounded-2xl border border-border bg-surface/95 backdrop-blur-md p-4 shadow-[0_20px_50px_rgba(0,0,0,0.15)] w-56 overflow-exempt animate-fade-in text-txt-primary flex flex-col gap-2.5"
 :class="[
 ...
 <span class="text-[10px] font-bold uppercase tracking-wider text-txt-disabled flex items-center gap-1">
 <span class="text-emerald-500">✓</span> Resume Health
 </span>
 <button 
 type="button" 
 class="shrink-0 rounded-full p-1 hover:bg-surface-hover text-txt-disabled transition-colors"
 ...
 <span class="text-2xl font-extrabold text-txt-primary tracking-tight">{{ healthScore }}%</span>
 ...
 <div class="text-[10px] text-txt-muted flex items-center justify-between border-t border-border pt-2">
 <span>Percentile Rating</span>
 <span class="font-semibold text-txt-primary" v-if="healthScore >= 80">Top 10%</span>
 <span class="font-semibold text-txt-primary" v-else-if="healthScore >= 50">Top 40%</span>
 <span class="font-semibold text-txt-primary" v-else>Top 80%</span>
 </div>
 </div>
 </Transition>
 ...
 <button
 type="button"
 class="flex size-full items-center justify-between gap-1.5 rounded-full bg-surface/90 backdrop-blur-md shadow-lg border border-border/80 px-3 py-2 transition hover:scale-105 active:scale-95 cursor-grab active:cursor-grabbing hover:border-brand/40"
 @mousedown="startDrag"
 @touchstart="startDrag"
 @click="handleFabClick"
 >
 <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-background border border-border text-[10px] font-bold" :class="scoreColorClass">✓</span>
 <div class="flex flex-col items-start leading-none text-left flex-1 min-w-0">
 <span class="text-[8px] font-bold text-txt-disabled uppercase tracking-wider">Health</span>
 <span class="text-xs font-extrabold text-txt-primary">{{ healthScore }}%</span>
 </div>
 </button>
 </div>
 </Transition>
