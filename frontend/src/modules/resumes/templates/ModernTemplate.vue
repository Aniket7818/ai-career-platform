<template>
 ...
 <p class="font-semibold text-txt-primary">{{ exp.role }}</p>
 ...
 <div class="flex flex-wrap items-baseline justify-between gap-2">
 <p class="font-semibold text-txt-primary">{{ edu.school }}</p>
 ...
 <p class="font-semibold text-txt-primary">{{ proj.projectName }}</p>
 <a v-if="proj.projectLink" :href="proj.projectLink" class="text-[0.786em]" :style="{ color: primaryHex }">Link ↗</a>
 </div>
 <p v-if="proj.technologies" class="text-[0.786em] font-medium text-txt-disabled">{{ proj.technologies }}</p>
 <p v-if="proj.description" class="mt-1 text-[0.857em] leading-6 text-txt-secondary">{{ proj.description }}</p>
 </div>
 </section>
