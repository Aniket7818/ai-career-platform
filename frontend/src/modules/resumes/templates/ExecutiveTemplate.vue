    ...
    <div class="px-8 py-6 space-y-5">
      <template v-for="section in visibleSections" :key="section">
        ...
        <section v-if="section === 'summary' && personal.summary">
          ...
          <p class="mt-2 text-[0.929em] leading-7 text-slate-700">{{ personal.summary }}</p>
        </section>
        ...
        <section v-if="section === 'experience' && experiences.length">
          ...
          <div v-for="exp in experiences" :key="exp.id || exp.role" class="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
            ...
            <p class="mt-0.5 font-medium text-slate-600">{{ exp.company }}</p>
            <p v-if="exp.description" class="mt-1.5 text-[0.857em] text-slate-600">{{ exp.description }}</p>
          </div>
        </section>
        ...
        <section v-if="section === 'skills' && skills.skillList">
          ...
          <p class="mt-2 text-slate-700">{{ skills.skillList }}</p>
        </section>
        ...
        <section v-if="section === 'certifications' && certifications.length">
          ...
          <div class="mt-3 flex flex-wrap gap-3">
            <div v-for="cert in certifications" :key="cert.id || cert.certName" class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
        ...
        <section v-if="section === 'achievements' && achievements.length">
          ...
          <ul class="mt-2 space-y-1 list-disc pl-4 text-[0.857em] text-slate-700">
        ...
        <section v-if="section === 'interests' && interests.length">
          ...
          <p class="mt-2 text-[0.857em] text-slate-600">{{ interests.join(' · ') }}</p>
        </section>
      </template>
    </div>
  </div>
