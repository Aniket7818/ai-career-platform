<template>
  ...
  >
    <header class="border-b-4 px-8 py-8" :style="{ borderColor: primaryHex, background: `linear-gradient(to right, ${primaryHex}0d, transparent)` }">
      ...
      <div v-if="contacts.length" class="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-[0.786em] text-slate-500">
        ...
        <section v-if="section === 'interests' && interests.length">
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            <span
              ...
              :key="interest"
              class="rounded-md px-2.5 py-1 text-[0.786em] font-medium text-slate-600 bg-slate-100"
            >{{ interest }}</span>
          </div>
        </section>
      </template>
