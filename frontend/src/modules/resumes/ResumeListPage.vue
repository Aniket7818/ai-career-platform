<template>
  <AppShell>
    <section class="space-y-8">
      ...
      <div v-else-if="!resumes.length" class="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center">
        ...
        <p class="mt-5 text-lg font-semibold text-ink">{{ t('resumes.empty') }}</p>
        <p class="mt-2 text-sm text-slate-500">{{ t('resumes.emptyHint') }}</p>
      ...
      <div v-else class="grid gap-4">
        <article
          ...
          :key="resume.id"
          class="group rounded-2xl border border-slate-100 bg-white p-5 shadow-panel transition hover:border-brand/30"
        >
          <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex items-start gap-4">
              ...
              <div>
                <h2 class="text-lg font-bold text-ink">{{ resume.title }}</h2>
                <div class="mt-2 flex flex-wrap items-center gap-3">
                  <ResumeStatusBadge :status="resume.status" />
                  <span class="text-xs text-slate-500">{{ formatDate(resume.updated_at) }}</span>
                </div>
              </div>
            </div>
            <div class="flex flex-wrap gap-2">
              <RouterLink class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-ink transition hover:bg-slate-50" :to="`/resumes/${resume.id}/edit`">
