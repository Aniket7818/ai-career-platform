      ...
      <div v-else class="grid gap-4">
        ...
        >
          <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="grid grid-cols-2 gap-2 w-full lg:flex lg:w-auto lg:gap-2">
              <RouterLink class="col-span-2 lg:col-span-1 inline-flex items-center justify-center gap-2 rounded-xl border border-border px-4 py-2 text-sm font-semibold text-txt-primary transition hover:bg-background" :to="`/resumes/${resume.id}/edit`">
                <svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002-2v-7" /><path d="M18.5 2.5a2.12 2.12 0 013 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                <span>{{ t('resumes.edit') }}</span>
              </RouterLink>
              <RouterLink class="inline-flex items-center justify-center gap-2 rounded-xl border border-brand/30 bg-brand/5 px-4 py-2 text-sm font-semibold text-brand transition hover:bg-brand/10" :to="`/resumes/${resume.id}/optimize`">
              ...
              <button class="inline-flex items-center justify-center gap-2 rounded-xl bg-red-500/10 px-4 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-500/20" @click="remove(resume.id)">
