<template>
 <main class="grid min-h-screen bg-surface lg:grid-cols-[1.1fr_1fr]">
 ...
 <p class="mt-5 text-lg leading-8 text-txt-muted">{{ t('auth.heroSubtitle') }}</p>

 <div class="mt-10 grid gap-4 sm:grid-cols-2">
 <div v-for="item in highlights" :key="item.title" class="rounded-2xl border border-white/10 bg-surface/5 p-4 backdrop-blur">
 <p class="text-sm font-bold text-mint">{{ item.stat }}</p>
 <p class="mt-1 font-semibold">{{ item.title }}</p>
 <p class="mt-1 text-xs text-txt-disabled">{{ item.body }}</p>
 </div>
 </div>

 <div class="mt-8 rounded-2xl border border-white/10 bg-surface/5 p-5">
 <p class="text-sm italic text-txt-muted">"{{ t('landing.testimonial') }}"</p>
 <p class="mt-3 text-xs font-semibold uppercase tracking-wider text-txt-muted">{{ t('auth.trustedBy') }}</p>
 </div>
 </div>

 <div class="relative mt-8 grid grid-cols-3 gap-3">
 <div v-for="stat in stats" :key="stat.label" class="rounded-xl border border-white/10 bg-surface/5 p-3 text-center">
 <p class="text-xl font-bold text-brand">{{ stat.value }}</p>
 <p class="mt-1 text-[10px] uppercase tracking-wider text-txt-disabled">{{ stat.label }}</p>
 </div>
 </div>
 </section>
 ...
 <h1 class="text-3xl font-bold text-txt-primary">{{ t('auth.loginTitle') }}</h1>
 <p class="text-sm text-txt-muted">{{ t('auth.loginSubtitle') }}</p>
 ...
 <button type="button" class="absolute right-3 text-txt-disabled hover:text-txt-secondary focus:outline-none" @click="showLoginPassword = !showLoginPassword">
 ...
 <h1 class="text-3xl font-bold text-txt-primary">{{ t('auth.signupTitle') }}</h1>
 <p class="text-sm text-txt-muted">{{ t('auth.signupSubtitle') }}</p>
 <ErrorState v-if="isSignup && error" :message="error" />
 <div class="grid gap-4 sm:grid-cols-2">
 <div><label>{{ t('auth.firstName') }}</label><input v-model="signupForm.first_name" required /></div>
 <div><label>{{ t('auth.lastName') }} <span class="normal-case text-txt-disabled">({{ t('auth.optional') }})</span></label><input v-model="signupForm.last_name" /></div>
 </div>
 ...
 <button type="button" class="absolute right-3 text-txt-disabled hover:text-txt-secondary focus:outline-none" @click="showSignupPassword = !showSignupPassword">
