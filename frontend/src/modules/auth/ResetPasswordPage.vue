<template>
  <AuthLayout>
    ...
    <div v-else-if="invalidToken" class="w-full max-w-md space-y-6 text-center">
      ...
      <h1 class="text-2xl font-bold text-ink">{{ t('auth.resetErrorInvalidToken') }}</h1>
    ...
    <form v-else class="w-full max-w-md space-y-4" @submit.prevent="submit">
      ...
      <p class="text-sm text-slate-500">{{ t('auth.resetSubtitle') }}</p>
      ...
      <RouterLink class="block text-center text-sm font-semibold text-slate-500 hover:text-ink" to="/login">
        {{ t('auth.hasAccount') }}
      </RouterLink>
    </form>
  </AuthLayout>
