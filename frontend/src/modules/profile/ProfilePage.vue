<template>
 ...
 <h2 class="truncate text-2xl font-bold text-txt-primary">{{ form.first_name }} {{ form.last_name }}</h2>
 <p class="truncate text-sm text-txt-muted">@{{ form.username }}</p>
 ...
 <div><label>{{ t('profile.email') }}</label><input :value="form.email" disabled class="bg-background text-txt-muted" /></div>
 <div><label>{{ t('auth.username') }}</label><input :value="form.username" disabled class="bg-background text-txt-muted" /></div>
