<template>
  <AppShell>
    ...
    <div class="max-w-7xl mx-auto space-y-12 animate-fade-in pb-16 px-4 sm:px-6 lg:px-8 mt-8">
      ...
      <div class="flex flex-col xl:flex-row gap-8 relative z-10">
        ...
        <div class="flex-1 space-y-8">
          <div>
            ...
            <h1 class="text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight mb-2 leading-tight">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <button @click="showToast('Total Questions filter coming soon')" class="text-left bg-white rounded-2xl p-5 border border-slate-200/60 shadow-sm hover:border-brand/30 transition-colors group focus:outline-none focus:ring-4 focus:ring-brand/20">
              <div class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
              ...
              <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Total Questions</p>
              <p class="text-2xl font-black text-slate-900">{{ totalQuestions }}</p>
            </button>
            <RouterLink :to="{ name: 'interview-prep-completed' }" class="text-left bg-white rounded-2xl p-5 border border-slate-200/60 shadow-sm hover:border-emerald-300 transition-colors group focus:outline-none focus:ring-4 focus:ring-emerald-200 block">
              ...
              <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Completed</p>
              <p class="text-2xl font-black text-slate-900">{{ totalCompletedCount }}</p>
            </RouterLink>
            <RouterLink :to="{ name: 'interview-prep-bookmarks' }" class="text-left bg-white rounded-2xl p-5 border border-slate-200/60 shadow-sm hover:border-amber-300 transition-colors group focus:outline-none focus:ring-4 focus:ring-amber-200 block">
              ...
              <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Bookmarks</p>
              <p class="text-2xl font-black text-slate-900">{{ totalBookmarkedCount }}</p>
            </RouterLink>
            <button @click="showToast('Learning Statistics page coming soon')" class="text-left bg-white rounded-2xl p-5 border border-slate-200/60 shadow-sm hover:border-purple-300 transition-colors group focus:outline-none focus:ring-4 focus:ring-purple-200">
              ...
              <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Remaining Time</p>
              <p class="text-2xl font-black text-slate-900">~{{ remainingEstTime }}h</p>
            </button>
          </div>
          ...
          <div class="flex flex-wrap gap-3 pt-2">
            ...
            <RouterLink :to="{ name: 'interview-prep-bookmarks' }" class="inline-flex items-center gap-2 bg-white text-slate-700 border border-slate-200 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-50 transition-colors">
            ...
            <RouterLink :to="{ name: 'interview-prep-completed' }" class="inline-flex items-center gap-2 bg-white text-slate-700 border border-slate-200 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-50 transition-colors">
            ...
            <RouterLink :to="{ name: 'interview-prep-history' }" class="inline-flex items-center gap-2 bg-white text-slate-700 border border-slate-200 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-50 transition-colors">
          ...
          <div class="bg-slate-900 rounded-3xl p-6 md:p-8 text-white relative overflow-hidden shadow-xl border border-slate-800">
            ...
            <div class="relative z-10">
              <div class="flex items-center justify-between mb-8">
                <h3 class="text-sm font-bold text-slate-400 uppercase tracking-widest">Overall Progress</h3>
                <span class="bg-white/10 px-3 py-1 rounded-lg text-xs font-bold">{{ globalProgressPercentage }}%</span>
              </div>
              ...
              <div class="flex items-center justify-between text-sm font-semibold">
                <span class="text-white">{{ totalCompletedCount }} Solved</span>
                <span class="text-slate-500">{{ Math.max(0, totalQuestions - totalCompletedCount) }} Left</span>
              </div>
            </div>
          </div>

          <!-- Continue Learning Widget -->
          <div v-if="continueLearning" class="bg-white rounded-3xl p-6 border border-slate-200/60 shadow-lg relative overflow-hidden group hover:border-brand/40 transition-colors">
            ...
            <div class="flex items-center gap-2 mb-4">
              ...
              <h3 class="text-sm font-bold text-slate-900">Continue Learning</h3>
            </div>
            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{{ continueLearning.subjectName }}</p>
            <h4 class="text-lg font-black text-slate-900 mb-6 truncate" title="Resume last viewed question">Question {{ continueLearning.currentIndex + 1 }} of {{ continueLearning.totalQuestions }}</h4>
            <div class="w-full h-1 bg-slate-100 rounded-full overflow-hidden mb-4">
            ...
            <RouterLink 
              ...
              class="w-full flex items-center justify-center bg-slate-900 text-white py-3 rounded-xl text-sm font-bold shadow-md hover:bg-brand transition-colors focus:outline-none focus:ring-4 focus:ring-brand/20"
            >
              Resume Learning
      ...
      <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-8 pt-8 border-t border-slate-100">
        ...
        <div v-if="recentlyViewed.length > 0" class="flex-1 min-w-0">
          ...
          <div class="flex gap-4 overflow-x-auto pb-4 snap-x hide-scrollbar">
            <RouterLink 
              ...
              class="snap-start shrink-0 w-80 bg-white border border-slate-200/60 rounded-2xl p-5 hover:border-brand/40 hover:shadow-xl hover:shadow-brand/5 transition-all group focus:outline-none focus:ring-4 focus:ring-brand/20 flex flex-col justify-between"
            >
              <div>
                 <div class="flex items-center justify-between gap-2 mb-3">
                   <span class="text-[10px] font-black uppercase tracking-wider text-slate-500 bg-slate-100 px-2 py-1 rounded">{{ item.subjectSlug }}</span>
                 ...
                 <p class="text-base font-bold text-slate-900 line-clamp-2 leading-tight group-hover:text-brand transition-colors mb-4">{{ item.title }}</p>
              </div>
              <div class="flex items-center justify-between border-t border-slate-100 pt-3 mt-auto">
                 <span class="text-[11px] font-bold text-slate-400">Active</span>
        ...
        <div class="w-full lg:w-80 shrink-0">
          <div class="relative group">
            ...
            <input 
              v-model="searchQuery" 
              type="text" 
              class="w-full bg-white border border-slate-200/80 rounded-2xl pl-11 pr-4 py-4 text-sm font-medium text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand shadow-sm hover:border-slate-300 transition-all"
              placeholder="Search subjects..."
              aria-label="Search subjects"
            >
            <div class="absolute inset-y-0 right-0 pr-2 flex items-center pointer-events-none">
              <kbd class="hidden sm:inline-flex items-center px-2 py-1 rounded bg-slate-100 border border-slate-200 text-[10px] font-bold text-slate-500">⌘K</kbd>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pt-4">
        <div v-for="i in 6" :key="i" class="h-64 bg-slate-100 animate-pulse rounded-3xl border border-slate-200/50"></div>
      </div>
        ...
        <div v-for="sub in filteredSubjects" :key="sub.slug" 
          ...
          <div class="relative z-10 flex-1">
            <div class="flex justify-between items-start mb-6">
              <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200/50 flex items-center justify-center shadow-sm group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
              ...
              <div class="flex items-center justify-center w-14 h-14 rounded-full border-[5px] border-slate-50 relative shadow-sm bg-white" title="Subject Progress">
            ...
            <h2 class="text-2xl font-extrabold text-slate-900 mb-3 group-hover:text-brand transition-colors">{{ sub.subject }}</h2>
            
            <div class="flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500 mb-6">
              <span class="flex items-center gap-1.5 bg-slate-100 text-slate-700 px-2.5 py-1 rounded-md" title="Total Questions">
              ...
              <span class="flex items-center gap-1.5 bg-slate-100 text-slate-700 px-2.5 py-1 rounded-md" title="Estimated Time">
          ...
          <div class="mt-auto pt-6 border-t border-slate-100 flex items-center justify-between">
            <span class="text-sm font-bold text-slate-400 group-hover:text-slate-600 transition-colors">Start Prep</span>
            <button class="inline-flex items-center justify-center bg-slate-900 text-white px-6 py-3.5 rounded-xl text-sm font-bold group-hover:bg-brand transition-all shadow-md group-hover:shadow-brand/30 group-hover:scale-[1.02] active:scale-95" aria-label="Continue learning {{ sub.subject }}">
      ...
      <!-- Empty State -->
      <div v-else class="bg-white rounded-3xl border border-dashed border-slate-300 p-16 text-center max-w-2xl mx-auto mt-12 shadow-sm">
        ...
        <h3 class="text-xl font-extrabold text-slate-900 mb-2">No subjects found</h3>
        <p class="text-base text-slate-500 max-w-md mx-auto">We couldn't find any subjects matching your search criteria. Try a different term.</p>
        <button @click="searchQuery = ''" class="mt-6 inline-flex items-center justify-center bg-white border-2 border-slate-200 text-slate-700 px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-slate-50 hover:border-slate-300 transition-all focus:outline-none focus:ring-4 focus:ring-slate-100">
          Clear Search
        </button>
      </div>
    </div>
