<template>
  <AppShell>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in relative">
      ...
      <!-- Breadcrumbs & Header -->
      <div class="mb-10">
        <nav class="flex text-xs font-bold text-slate-400 mb-4 uppercase tracking-wider" aria-label="Breadcrumb">
          <ol class="flex flex-wrap items-center gap-2 sm:gap-2.5">
            ...
            <li class="flex items-center">
              <span class="text-slate-800 leading-none pt-0.5">{{ subjectTitle }}</span>
            </li>
          </ol>
        </nav>
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 class="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight">{{ subjectTitle }}</h1>
            <p class="text-lg text-slate-500 mt-2 font-medium">{{ questions.length }} questions available</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="lg:w-72 shrink-0">
          <div class="sticky top-20 bg-white/80 backdrop-blur-xl p-6 rounded-3xl border border-slate-200/60 shadow-sm z-20 space-y-6">
            <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">Filters</h3>
            
            <div class="space-y-5">
              <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Topic</label>
                <div class="relative group">
                  <select v-model="selectedTopic" class="appearance-none w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-semibold rounded-xl pl-4 pr-10 py-3.5 focus:ring-4 focus:ring-brand/10 focus:border-brand focus:bg-white outline-none transition-all shadow-sm cursor-pointer hover:border-slate-300">
                    <option value="">All Topics</option>
                    <option v-for="topic in uniqueTopics" :key="topic" :value="topic">{{ topic }}</option>
                  </select>
                  <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-slate-400 group-hover:text-brand transition-colors">
              ...
              <div>
                ...
                <div class="relative group">
                  <select v-model="selectedDifficulty" class="appearance-none w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-semibold rounded-xl pl-4 pr-10 py-3.5 focus:ring-4 focus:ring-brand/10 focus:border-brand focus:bg-white outline-none transition-all shadow-sm cursor-pointer hover:border-slate-300">
            ...
            <button v-if="selectedTopic || selectedDifficulty" @click="selectedTopic = ''; selectedDifficulty = ''" class="w-full py-2 text-xs font-bold text-slate-500 hover:text-slate-800 transition-colors">
              Reset Filters
            </button>
          </div>
        </div>
        ...
        <div class="flex-1 min-w-0 pb-20">
          <div v-if="loading" class="space-y-4">
            <div v-for="i in 5" :key="i" class="h-32 bg-slate-100 animate-pulse rounded-3xl border border-slate-200/50"></div>
          </div>
          
          <div v-else-if="questions.length" class="space-y-4">
            <div v-if="filteredQuestions.length === 0" class="bg-white rounded-3xl border border-dashed border-slate-300 p-16 text-center shadow-sm">
              ...
              <p class="text-slate-800 font-bold text-lg mb-1">No matches found</p>
              <p class="text-slate-500 text-sm mb-6">Try adjusting your filters to see more results.</p>
              <button @click="selectedTopic = ''; selectedDifficulty = ''" class="bg-slate-900 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md hover:bg-brand transition-colors">Clear all filters</button>
            </div>
            ...
            <RouterLink 
              ...
              class="group relative block bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-200 hover:shadow-xl hover:border-slate-300 transition-all duration-300 transform hover:-translate-y-1 overflow-hidden"
            >
              ...
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pl-2">
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-3 mb-3">
                    ...
                    <span class="inline-flex items-center text-[11px] font-bold text-slate-500 bg-slate-100 rounded-md px-2 py-0.5 border border-slate-200/50">
                      {{ q.topic }}
                    </span>
                    <span class="hidden sm:inline-flex items-center text-[11px] font-bold text-slate-400 ml-auto">
                  ...
                  <h3 class="text-xl font-bold text-slate-900 group-hover:text-brand transition-colors line-clamp-2 leading-tight pr-4">
                    {{ q.title }}
                  </h3>
                </div>
                
                <div class="shrink-0 flex items-center justify-between sm:justify-end border-t sm:border-t-0 border-slate-100 pt-4 sm:pt-0">
                  <div class="flex items-center sm:hidden text-[11px] font-bold text-slate-400">
                    <svg class="w-3.5 h-3.5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    ~3 min
                  </div>
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    <!-- Bookmark Icon -->
                    <button class="p-1.5 rounded-lg transition-colors" :class="isBookmarked(route.params.subject, q.id) ? 'text-amber-500 bg-amber-50' : 'text-slate-300 hover:text-amber-500 hover:bg-slate-50'" @click="toggleBookmark($event, q.id)" title="Bookmark">
          ...
          <div v-else class="bg-white rounded-3xl border border-dashed border-slate-300 p-16 text-center max-w-2xl mx-auto shadow-sm">
            ...
            <p class="text-slate-900 font-extrabold text-xl mb-2">No questions available</p>
            <p class="text-slate-500 mb-8">We're currently adding more content for this subject.</p>
            <RouterLink :to="{ name: 'interview-prep' }" class="inline-flex items-center justify-center bg-slate-900 text-white px-6 py-3 rounded-xl text-sm font-bold shadow-md hover:bg-brand transition-colors">
