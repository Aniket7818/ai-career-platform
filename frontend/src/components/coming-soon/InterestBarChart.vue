<template>
 ...
 <span class="text-xs font-semibold text-txt-muted opacity-0 transition group-hover:opacity-100">{{ item.count }}</span>
 <div class="relative w-full overflow-hidden rounded-t-md bg-surface/5" :style="{ height: `${barHeight(item.count)}%` }">
 <div
 class="absolute inset-0 rounded-t-md bg-gradient-to-t from-brand/80 to-brand transition-all duration-700"
 :style="{ animationDelay: `${index * 80}ms` }"
 />
 </div>
 <span class="max-w-full truncate text-[10px] font-medium text-txt-muted" :title="item.label">{{ item.shortLabel }}</span>
 </div>
 </div>
</template>
