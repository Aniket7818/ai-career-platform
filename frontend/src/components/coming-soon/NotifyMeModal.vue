<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-md rounded-2xl border border-white/10 bg-surface-light p-6 shadow-glow">
      ...
      <p class="mt-2 text-sm text-slate-400">{{ t('comingSoon.modal.subtitle') }}</p>
      <p v-if="featureTitle" class="mt-3 rounded-lg bg-brand/10 px-3 py-2 text-sm font-semibold text-brand">{{ featureTitle }}</p>
      <form class="mt-5 space-y-4" @submit.prevent="submit">
        <div>
          <label class="text-xs font-semibold uppercase tracking-wide text-slate-400">{{ t('auth.email') }}</label>
          <input v-model="email" type="email" required class="mt-1.5 w-full rounded-xl border border-white/10 bg-surface px-4 py-2.5 text-white placeholder:text-slate-500" :placeholder="t('comingSoon.modal.emailPlaceholder')" />
        </div>
        <ErrorState v-if="error" :message="error" />
        <div class="flex gap-3">
          <button type="button" class="flex-1 rounded-xl border border-white/10 px-4 py-2.5 text-sm font-semibold text-slate-300 hover:bg-white/5" @click="$emit('close')">{{ t('common.cancel') }}</button>
