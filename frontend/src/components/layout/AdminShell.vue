    ...
    >
      ...
      <nav class="flex-1 space-y-1 overflow-y-auto p-4">
        <a
          ...
          :class="activeSection === item.id ? 'bg-brand/20 text-white ring-1 ring-brand/40' : 'text-slate-400 hover:bg-white/5 hover:text-white'"
          @click.prevent="scrollTo(item.id)"
        >
          <component :is="item.icon" class="size-4 shrink-0" />
          {{ item.label }}
        </a>
        <RouterLink class="mt-4 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-400 transition hover:bg-white/5 hover:text-white" to="/dashboard" @click="close">
        ...
        <div class="flex flex-col gap-3 sm:gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div class="flex items-center gap-3">
            ...
            <div class="relative min-w-0 flex-1 lg:max-w-md">
              <svg class="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" /></svg>
              <input
                :value="searchQuery"
                class="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 pl-10 pr-4 text-sm text-white outline-none transition placeholder:text-slate-500 focus:border-brand/50 focus:ring-2 focus:ring-brand/20"
