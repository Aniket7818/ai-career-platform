<template>
  <div class="issue-card" :class="priorityClass">
    ...
    <div class="issue-actions">
      ...
      <button class="btn-secondary" @click="$emit('dismiss')">Dismiss</button>
    </div>
  </div>
</template>
...
.gain { color: #10b981; }
.time { color: var(--color-text-muted); display: flex; align-items: center; gap: 0.25rem; }
.time::before { content: '⏱'; font-size: 0.8rem; }
...
.btn-secondary:hover { background: var(--color-surface-2); color: var(--color-text-primary); }
